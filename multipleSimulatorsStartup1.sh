...
path=$(find ~/Library/Developer/Xcode/DerivedData/Maby-*/Build/Products/Debug-iphonesimulator -name "Joyful.app" | head -n 1)
echo "${path}"
...
xcrun simctl launch "$line" io.sleepyfran.Maby # Launch .app using its bundle at simulator with identifier hold by $line var
done
